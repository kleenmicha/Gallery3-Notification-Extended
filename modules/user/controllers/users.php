...
class Users_Controller extends Controller {
 ...
 // Persoenliche Einstellungen - Anfang
 public function change_setting($id) {
    ...
    $form = $this->_get_change_setting_form($user);
    try {
      $valid = $form->validate();
      $user->emailtype = $form->change_setting->emailtype->value;
      $user->validate();
    } catch (ORM_Validation_Exception $e) {
      // Translate ORM validation errors into form error messages
      foreach ($e->validation->errors() as $key => $error) {
        $form->change_setting->inputs[$key]->add_error($error, 1);
      }
      $valid = false;
    }
    ...
    if ($valid) {
      $user->save();
      module::event("user_change_setting_form_completed", $user, $form);
      message::success(t("Settings changed"));
      module::event("user_auth", $user);
      json::reply(array("result" => "success",
                        "resource" => url::site("users/{$user->id}")));
    } else {
      log::warning("user", t("Failed Setting change for %name", array("name" => $user->name)));
      $name = $user->name;
      module::event("user_auth_failed", $name);
      json::reply(array("result" => "error", "html" => (string)$form));
    }
  }
    // Persoenliche Einstellungen - Ende
  ...
  // Persoenliche Einstellungen - Anfang
  public function form_change_setting($id) {
    $user = user::lookup($id);
    if (!$user || $user->guest || $user->id != identity::active_user()->id) {
      access::forbidden();
    }

    print $this->_get_change_setting_form($user);
  }
   // Persoenliche Einstellungen - Ende
  ...
  // Persoenliche Einstellungen - Anfang
  private function _get_change_setting_form($user) {
    $form = new Forge(
      "users/change_setting/$user->id", "", "post", array("id" => "g-change-setting-user-form"));
    $group = $form->group("change_setting")->label(t("Change your personal settings"));
    $group->dropdown("emailtype")->label(t("Change Emailtype"))
	  ->options(array("0" => t("html Email"),"1" => t("Text Email")))
	  ->selected($user->emailtype);
	
    module::event("user_change_setting_form", $user, $form);
    $group->submit("")->value(t("Save"));
    return $form;
  }
  // Persoenliche Einstellungen - Ende
