 ...
 */
class notification_ext_event_Core {
  ...
  static function item_created($item) {
    try {
      notification_ext::send_item_add($item);
    } catch (Exception $e) {
  ...
  static function item_deleted($item) {
    try {
      notification_ext::send_item_deleted($item);

      if (notification_ext::is_watching($item)) {
        notification_ext::remove_watch($item);
      }
    } catch (Exception $e) {
  ...
  static function comment_created($comment) {
    try {
      if ($comment->state == "published") {
        notification_ext::send_comment_published($comment);
      }
    } catch (Exception $e) {
  ...
  static function comment_updated($original, $new) {
    try {
      if ($new->state == "published" && $original->state != "published") {
        notification_ext::send_comment_published($new);
      }
    } catch (Exception $e) {
  ...
  static function batch_complete() {
    try {
      notification_ext::send_pending_notifications_ext();
    } catch (Exception $e) {
  ...
  static function site_menu($menu, $theme) {
    if (!identity::active_user()->guest) {
      ...
      if ($item && $item->is_album() && access::can("view", $item)) {
        $watching = notification_ext::is_watching($item);
        ...
        $menu->get("options_menu")
          ->append(Menu::factory("link")
                   ...
                   ->url(url::site("notification_ext/watch/$item->id?csrf=" . access::csrf_token())));
      }
    }
  }
