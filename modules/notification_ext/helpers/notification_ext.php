...
class notification_ext {
  ...
  static function remove_watch($item, $user=null) {
	...
	static function get_subscribers($item) {
		...
		foreach ($users as $user) {
			if (access::user_can($user, "view", $item) && !empty($user->email)) {
				$subscribers[$user->email] = $user->emailtype.$user->locale;
			}
		}
		return $subscribers;
	}
	...
	static function send_item_updated($original, $item) {
		foreach (self::get_subscribers($item) as $email => $local) {
			$emailtype = substr($local,0,1);
			$locale = substr($local,1);
			if($emailtype == 0) $v = new View("item_updated.html");
			else  $v = new View("item_updated.txt");
			$v->original = $original;
			$v->item = $item;
			...
			self::_notify($email, $locale, $item, $v->render(), $v->subject, $emailtype);
		}
	}
	...
	static function send_item_add($item) {
		...
		foreach (self::get_subscribers($item) as $email => $local) {
			$emailtype = substr($local,0,1);
			$locale = substr($local,1);
			if($emailtype == 0) $v = new View("item_added.html");
			else  $v = new View("item_added.txt");
			$v->item = $item;
			$v->subject = $item->is_album() ?
	...
	static function send_item_deleted($item) {
		...
		foreach (self::get_subscribers($item) as $email => $local) {
			$emailtype = substr($local,0,1);
			$locale = substr($local,1);
			if($emailtype == 0) $v = new View("item_deleted.html");
			else  $v = new View("item_deleted.txt");
			$v->item = $item;
			$v->subject = $item->is_album() ?
 	...
 	static function send_comment_published($comment) {
		...
		foreach (self::get_subscribers($item) as $email => $local) {
			$emailtype = substr($local,0,1);
			$locale = substr($local,1);
			if($emailtype == 0) $v = new View("comment_published.html");
			else  $v = new View("comment_published.txt");
			$v->comment = $comment;
			$v->subject = $item->is_album() ?
	...
	static function send_pending_notifications_ext() {
    	foreach (db::build()
	...
	private static function _notify($email, $locale, $item, $text, $subject, $emailtype) {
